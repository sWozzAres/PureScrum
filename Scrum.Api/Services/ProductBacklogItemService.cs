...
namespace PureScrum.Services
{
    public class ProductBacklogItemService(ScrumDbContext dbContext, ILogger<ProductBacklogItemService> logger) : ProductBacklogItemServiceBase
    {
        ...
        public override async Task<ListProductBacklogItemsResponse> List(ListProductBacklogItemsRequest request, ServerCallContext context)
        {
            ...
            var query = from q in baseQuery
                        ...
                        (
                            q.pbi.Id,
                            q.pbi.Name,
                            (ScrumApi.PbiStatus)q.pbi.Status,
                            q.s == null ? "" : q.s.Name,
                            q.p.Name,
            ...
            {
                var pbi = new ScrumApi.ProductBacklogItem()
                {
                    Id = p.Id.ToString(),
                    Name = p.Name,
                    Status = p.Status,
        ...
        record PbiTemp(Guid Id, string Name, ScrumApi.PbiStatus Status, string SprintName, string ProductName,
        ...
        record PbiShortTemp(Guid Id, string Name, ScrumApi.PbiStatus Status);
    }
}
